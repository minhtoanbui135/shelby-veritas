  ...
  move
    .createObjectAndPublishPackage({
      ...
      namedAddresses: {
        ...
        veritas: process.env.NEXT_MODULE_PUBLISHER_ACCOUNT_ADDRESS,
      },
      extraArguments: [`--private-key=${process.env.NEXT_MODULE_PUBLISHER_ACCOUNT_PRIVATE_KEY}`,`--url=${aptosSDK.NetworkToNodeAPI[process.env.NEXT_PUBLIC_APP_NETWORK] ?? "https://api.shelbynet.shelby.xyz/v1"}`, "--assume-yes"],
    })
    .then((response) => {
      const filePath = ".env";
      let envContent = "";
